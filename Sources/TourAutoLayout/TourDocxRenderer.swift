...
struct TourDocxRenderer: DocxRenderer {
    func render(document: ImportedDocxSnapshot, template: TemplateProfile, outputURL: URL) throws {
        let backgroundImage = try ImagePreparation.preparedImage(fromFileURL: template.backgroundImageURL, preferredBaseName: "template-background")
...
private struct WordDocumentBuilder {
    ...
    func buildArchive(backgroundImage: PreparedImage) throws -> DocxArchiveDocument {
        ...
        let documentXML = makeDocumentXML(bodyInnerXML: document.bodyInnerXML)
        let stylesXML = document.stylesXML ?? makeFallbackStylesXML()
        let settingsXML = document.settingsXML ?? makeFallbackSettingsXML()
        let headerXML = makeHeaderXML()
        let headerRelationshipsXML = makeHeaderRelationshipsXML(backgroundPartPath: backgroundPart.path)
        let documentRelationshipsXML = makeDocumentRelationshipsXML()
        let contentTypesXML = makeContentTypesXML(backgroundPart: backgroundPart)
        let rootRelationshipsXML = makeRootRelationshipsXML()
        let coreXML = makeCoreXML()
        let appXML = makeAppXML()

        return DocxArchiveDocument(
            contentTypesXML: contentTypesXML,
            rootRelationshipsXML: rootRelationshipsXML,
            documentXML: documentXML,
            documentRelationshipsXML: documentRelationshipsXML,
            headerXML: headerXML,
            headerRelationshipsXML: headerRelationshipsXML,
            stylesXML: stylesXML,
    ...
    private func makeDocumentRelationshipsXML() -> String {
        ...
        if document.numberingXML != nil {
            relationships.append(
                ImportedRelationship(id: Self.numberingRelationshipID, type: RelationshipTypes.numbering, target: "numbering.xml", targetMode: nil)
            )
        }
        if document.fontTableXML != nil {
            relationships.append(
                ImportedRelationship(id: Self.fontTableRelationshipID, type: RelationshipTypes.fontTable, target: "fontTable.xml", targetMode: nil)
            )
        }
        if document.themeXML != nil {
            relationships.append(
                ImportedRelationship(id: Self.themeRelationshipID, type: RelationshipTypes.theme, target: "theme/theme1.xml", targetMode: nil)
            )
        }
    ...
    private func makeContentTypesXML(backgroundPart: ImportedMediaPart) -> String {
        let imageDefaults = Set((document.mediaParts + [backgroundPart]).map(\.contentType))
            .compactMap { contentType -> String? in
                switch contentType {
                case "image/png":
                    return "<Default Extension=\"png\" ContentType=\"image/png\"/>"
                case "image/jpeg":
                    return "<Default Extension=\"jpg\" ContentType=\"image/jpeg\"/><Default Extension=\"jpeg\" ContentType=\"image/jpeg\"/>"
                case "image/gif":
                    return "<Default Extension=\"gif\" ContentType=\"image/gif\"/>"
                case "image/bmp":
                    return "<Default Extension=\"bmp\" ContentType=\"image/bmp\"/>"
                case "image/tiff":
                    return "<Default Extension=\"tif\" ContentType=\"image/tiff\"/><Default Extension=\"tiff\" ContentType=\"image/tiff\"/>"
                default:
                    return nil
                }
            }
        ...
        let numberingOverride = document.numberingXML == nil ? "" : """
        <Override PartName="/word/numbering.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.numbering+xml"/>
        """

        let fontTableOverride = document.fontTableXML == nil ? "" : """
        <Override PartName="/word/fontTable.xml" ContentType="application/vnd.openxmlformats-officedocument.wordprocessingml.fontTable+xml"/>
        """

        let themeOverride = document.themeXML == nil ? "" : """
    ...
    private func makeAppXML() -> String {
        ...
        <Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes">
          <Application>TourAutoLayout</Application>
        </Properties>
        """
    }
...
struct ImagePreparation {
    fileprivate static func preparedImage(fromFileURL url: URL, preferredBaseName: String) throws -> PreparedImage {
        let data = try Data(contentsOf: url)
        return try preparedImage(data: data, originalFilename: url.lastPathComponent, preferredBaseName: preferredBaseName)
    }

    fileprivate static func preparedImage(data: Data, originalFilename: String, preferredBaseName: String) throws -> PreparedImage {
        let contentType = contentType(forFilename: originalFilename)
        let size = pixelSize(for: data)

        if ["image/png", "image/jpeg", "image/gif"].contains(contentType) {
            let ext = fileExtension(forContentType: contentType)
            return PreparedImage(data: data, contentType: contentType, fileExtension: ext, pixelSize: size)
        }

        guard let nsImage = NSImage(data: data),
              let representation = NSBitmapImageRep(data: nsImage.tiffRepresentation ?? Data()),
              let pngData = representation.representation(using: .png, properties: [:]) else {
            throw AppError.imageEncodingFailed
        }

        let pngSize = pixelSize(for: pngData)
        return PreparedImage(data: pngData, contentType: "image/png", fileExtension: "png", pixelSize: pngSize)
    }
